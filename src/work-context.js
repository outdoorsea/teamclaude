...
export class WorkContextStore {
  ...
  /**
   * Roll the ledger up into per-key totals over a recent window.
   *
   * @param {object} [opts]
   * @param {string} [opts.groupBy] - ledger field to bucket on, or 'account' / 'session'
   * @param {number} [opts.hours] - how far back to read
   * @param {{ projectSlug?: string, prdId?: number|string, beadId?: string }} [opts.filters]
   */
